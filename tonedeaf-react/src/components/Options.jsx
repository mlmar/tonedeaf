...
export const Options = ({
    ...
    return (
        <div
            className={'options-panel flex-col ' + (className || '')}
            {...props}
        >
            {title ? (
                <label className={'medium bold flex'} title={'Toggle Menu'}>
                    {title}
                </label>
            ) : null}
            ...
            {subtitle}
            <div
                className={'options ' + (col ? 'flex-col' : '')}
                onClick={handleClick}
            >
                {options?.map((option, i) => (
                    <Option selected={index === i} id={i} key={option + i}>
                        {renderOption(option)}
                    </Option>
                ))}
                {children}
...
export const Option = ({ id, selected, children, href }) => {
    return (
        <a
            id={id}
            className={
                'gray-btn bold flex option ' + (selected ? 'selected' : '')
            }
            href={href}
            target='_blank'
            rel='noreferrer'
