...
const ArtistSummary = (props) => {
    ...
    const topGenres = () => {
        if (artistsInfo?.genres?.length > 0) {
            ...
            const genres = artistsInfo?.genres
                .slice(1, DEFAULTS.TOP_GENRES_LIMIT + 1)
                .map((item, i) => (
                    <label
                        className="flex-col big bold"
                        key={i + item.genre}
                    ...
                    >
                        ...
                        <span className="bold">
                            {" "}
                            {((item.total / first.total) * 100).toFixed(
                                1
                            )}%{" "}
                        </span>
                    </label>
    ...
    const topArtists = () => {
        ...
        let style = {
            flexBasis:
                (DEFAULTS.TOP_IMAGES_ROWS / DEFAULTS.TOP_IMAGES_LIMIT) * 100 +
                "%",
        };
        ...
        return (
            <>
                <p className="panel large bold inactive">
                    {" "}
                    <span className="white"> {artist?.name} </span> was your
                    number one artist{" "}
                </p>
